...
# Allow ssh access from VM to Pi for mounting SSHFS shares
/ip firewall filter add action=accept chain=forward comment="Allow VM docker-host to Pi for managing files in code-server" dst-address=<PI_IP> dst-port=22 protocol=tcp src-address=<VM_IP>
```
